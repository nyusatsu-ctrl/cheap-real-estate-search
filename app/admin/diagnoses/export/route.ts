...
import { getStrategyAnswerLabel } from "@/lib/construction-diagnosis-v2/strategy";
  ...
  const rows = diagnoses.map((rawDiagnosis) => {
    if (isConstructionManagementDiagnosis(rawDiagnosis)) {
      ...
      return [
        ...
        formatNullableDate(diagnosis.short_started_at),
        diagnosis.short_last_step ?? "",
        formatNullableDate(diagnosis.detailed_started_at),
        diagnosis.detailed_last_step ?? "",
        diagnosis.abandoned_stage ?? "",
        diagnosis.abandoned_question_id ?? "",
        diagnosis.device_type ?? "",
        diagnosis.browser_family ?? "",
        diagnosis.diagnosis_status ? DIAGNOSIS_V2_PROGRESS_STATUS_LABELS[diagnosis.diagnosis_status] : "",
        diagnosis.detailed_answered_count,
        diagnosis.detailed_total_questions,
        diagnosis.detailed_last_question_id ?? "",
        diagnosis.detailed_current_step,
        formatNullableDate(diagnosis.last_saved_at),
        diagnosis.resume_token_created_at ? "発行済み" : "未発行",
        formatNullableDate(diagnosis.resume_token_expires_at),
        diagnosis.resume_count,
        ...
        formatNullableDate(diagnosis.strategy_started_at),
        formatNullableDate(diagnosis.strategy_completed_at),
        diagnosis.strategy_total_questions,
        diagnosis.strategy_question_ids.join(" / "),
        Object.entries(diagnosis.strategy_question_reasons).map(([id, reason]) => `${id}: ${reason}`).join(" / "),
        diagnosis.strategy_question_ids.map((id) => `${id}: ${getStrategyAnswerLabel(id, diagnosis.strategy_answers[id])}`).join(" / "),
        diagnosis.strategy_result ? [diagnosis.strategy_result.conclusion.firstAction, diagnosis.strategy_result.conclusion.priority, diagnosis.strategy_result.conclusion.potential].join(" / ") : "",
        diagnosis.strategy_growth_work.join(" / "),
        diagnosis.strategy_maintain_work.join(" / "),
        diagnosis.strategy_review_work.join(" / "),
        diagnosis.strategy_result?.actions30Days.join(" / ") ?? "",
        diagnosis.strategy_result ? [...diagnosis.strategy_result.plan90Days.month1, ...diagnosis.strategy_result.plan90Days.month2, ...diagnosis.strategy_result.plan90Days.month3].join(" / ") : "",
        diagnosis.strategy_monthly_metrics.join(" / "),
        diagnosis.strategy_result?.supportJudgment ?? "",
        diagnosis.property_search_interest ?? "",
        diagnosis.property_search_interest_topics.join(" / "),
        formatNullableDate(diagnosis.precheck_started_at),
        formatNullableDate(diagnosis.precheck_completed_at),
