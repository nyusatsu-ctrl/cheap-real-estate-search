...
function CandidateReview({ candidate }: { candidate: TenderCandidate }) {
  ...
  const deadline = assessTenderDeadline(candidate);
  return (
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <div className="flex flex-wrap gap-2">
            ...
            <span className={`rounded px-2 py-0.5 text-xs font-bold ${deadlineStatusBadgeClass(deadline.status)}`}>{deadline.label}</span>
          ...
          <div className="mt-2 grid gap-2 rounded border border-slate-200 bg-white p-3 text-xs leading-5 text-slate-700 md:grid-cols-2">
            <div>
              <p className="font-black text-slate-900">抽出された期限日: {deadline.deadlineAt ? formatDate(deadline.deadlineAt) : "-"}</p>
              <p className="mt-1 font-semibold">期限ステータス: {deadline.label}</p>
              <p className="mt-1 font-semibold">期限抽出元: {deadline.source ?? "-"}</p>
            </div>
            <div>
