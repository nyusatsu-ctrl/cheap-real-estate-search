    ...
    <div className="grid gap-4">
      <form action={saveTenderAction} className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
        ...
        <div className="grid gap-4 md:grid-cols-2">
          <Field label="案件名" name="title" defaultValue={tender?.title} required className="md:col-span-2" />
          <Field label="発注機関" name="agency_name" defaultValue={tender?.agency_name} required />
          <Select label="案件種別" name="tender_type" defaultValue={tender?.tender_type} options={TENDER_TYPE_LABELS} />
          ...
          <Field label="入札日または見積期限" name="bid_at" type="datetime-local" defaultValue={toDateTimeLocal(tender?.bid_at)} />
          ...
          <Checkbox label="締切間近フラグ" name="is_deadline_soon" defaultChecked={tender?.is_deadline_soon ?? false} />
          <Checkbox label="防衛省・自衛隊案件" name="is_defense" defaultChecked={tender?.is_defense ?? false} />
        </div>
      ...
      {tender ? (
        <form action={deleteTenderAction} className="rounded-lg border border-rose-200 bg-rose-50 p-4">
          <input type="hidden" name="id" value={tender.id} />
          <p className="text-sm font-semibold text-rose-800">この案件を削除します。重複統合の前に、公式URLとPDF URLが同一か確認してください。</p>
