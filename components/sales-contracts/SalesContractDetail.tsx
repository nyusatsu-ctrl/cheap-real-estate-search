        ...
        {item.guarantors.length ? (
          <div className="grid gap-3 md:grid-cols-2">
            {item.guarantors.map((guarantor) => (
              <div key={guarantor.id} className="min-w-0 rounded border border-slate-200 bg-slate-50 p-3 text-sm">
                <p className="break-words font-bold text-slate-950">{guarantor.name}</p>
                <p className="mt-1 break-words text-slate-600">{[guarantor.relationship, guarantor.phone].filter(Boolean).join(" / ") || "-"}</p>
          ...
          <div className="divide-y divide-slate-200 rounded border border-slate-200">
            {item.contactHistories.map((history) => (
              <div key={history.id} className="p-3 text-sm">
                ...
                <p className="mt-2 whitespace-pre-wrap break-words text-slate-700">{history.content}</p>
                <div className="mt-2 flex flex-wrap gap-2 text-xs font-semibold text-slate-500">
                  {history.next_action_date ? <span>次回対応: {formatSalesDate(history.next_action_date)}</span> : null}
                  {history.memo ? <span className="min-w-0 break-words">備考: {history.memo}</span> : null}
                </div>
              </div>
            ))}
          </div>
...
function DocumentLinkCard({
  ...
  return (
    <div className={`min-w-0 rounded border p-3 text-sm ${cardClass}`}>
      ...
      {document?.memo ? <p className="mt-2 whitespace-pre-wrap break-words text-xs font-semibold text-slate-600">備考: {document.memo}</p> : null}
    </div>
  );
}
        ...
        <div className="min-w-0">
          ...
          <dl className="mt-3 grid gap-3 text-sm sm:grid-cols-3">
            <div className="min-w-0">
              ...
              <dd className="mt-1 break-words font-black text-slate-950">自社ローン審査管理</dd>
            </div>
            <div className="min-w-0">
              <dt className="text-xs font-bold text-emerald-700">申込管理番号</dt>
              <dd className="mt-1 break-words font-black text-slate-950">{item.contract.source_row_number ?? "-"}</dd>
            </div>
            <div className="min-w-0">
              <dt className="text-xs font-bold text-emerald-700">受信日時</dt>
              <dd className="mt-1 break-words font-black text-slate-950">{formatSalesDateTime(item.contract.source_received_at)}</dd>
            </div>
          </dl>
        </div>
        <Link
...
function Info({
  ...
  return (
    <div className={`min-w-0 ${className}`}>
      ...
      <dd className={`mt-1 min-w-0 overflow-hidden break-words font-semibold text-slate-950 ${multiline ? "whitespace-pre-wrap" : "whitespace-normal"}`}>{isEmpty ? "-" : value}</dd>
    </div>
  );
}
...
function SummaryItem({ label, value }: { label: string; value: React.ReactNode }) {
  return (
    <div className="min-w-0 rounded border border-slate-200 bg-slate-50 px-3 py-2">
      <dt className="text-xs font-bold text-slate-500">{label}</dt>
      <dd className="mt-1 min-w-0 overflow-hidden break-words font-black text-slate-950">{value || "-"}</dd>
    </div>
  );
}
    ...
    <div className={`mt-5 rounded border px-4 py-3 ${status.panelClass}`}>
      ...
      {history ? (
        <div className="mt-2 min-w-0 text-sm">
          <p className="break-words font-bold text-slate-950">{formatSalesDate(history.next_action_date)} / {CONTACT_METHOD_LABELS[history.method]} / {CONTACT_STATUS_LABELS[history.status]}</p>
          <p className="mt-1 whitespace-pre-wrap break-words text-slate-700">{history.content}</p>
          {history.memo ? <p className="mt-1 whitespace-pre-wrap break-words text-xs font-semibold text-slate-500">備考: {history.memo}</p> : null}
        </div>
      ) : (
