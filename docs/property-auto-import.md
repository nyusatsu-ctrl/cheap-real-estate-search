...
- 現在の情報元: アットホーム 空き家バンク、みんなの0円物件、アキソル 0円物件、全国0円不動産、家いちば、ふるさと情報館、山いちば、熊本県空き家バンク、留萌市空き家情報バンク
- 対象価格: 0円から300万円以下
- 通常の登録状態: 公開中
...
```bash
npm run import:nationwide -- --max-price=3000000 --commit
```
...
みんなの0円物件は公開APIの `property.list` から募集中の0円物件を取得します。

アキソル 0円物件は受付中の物件一覧ページから詳細URLを取得し、詳細ページから所在地・面積・建築年などを取得します。

全国0円不動産は、公開ページに掲載された0円物件の詳細URLだけを取得します。成約済み表示があるものは登録対象から外します。

家いちばは、公開ページに埋め込まれている物件URLから詳細ページを確認し、0円から300万円以下の候補だけ登録します。

ふるさと情報館は、公開サイトマップの物件URLから詳細ページを確認し、0円から300万円以下の候補だけ登録します。

山いちばは、山林売買物件一覧から詳細ページを確認し、0円から300万円以下の山林候補だけ登録します。

LIFULL HOME'S 空き家バンクも国土交通省が案内する全国版ですが、通常のNode.js取得ではAWS WAFのチャレンジページになるため、自動収集元としては無効化しています。

裁判所競売、国税庁公売、官公庁オークション、国有財産、ジモティー、田舎ねっと.日本、不動産ジャパンは、検索画面・robots.txt・規約・取得方法の追加確認が必要なため、現時点では設定ファイルに「保留」として残し、自動収集では使っていません。

追加の保存項目をDBに保存するには、Supabase SQL Editorで次のファイルの内容を1回だけ実行してください。

```bash
supabase/property-import-metadata.sql
```

このSQLを実行すると、取引形態、掲載日、更新日、取得日時、価格帯、リスクタグ、備考を保存できるようになります。未実行でも取込自体は止まりませんが、追加項目は保存されません。
