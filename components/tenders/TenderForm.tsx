    ...
    <div className="grid gap-4">
      <form action={saveTenderAction} className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
        ...
        <div className="grid gap-4 md:grid-cols-2">
          ...
          <Select label="案件区分" name="tender_type" defaultValue={tender?.tender_type} options={TENDER_TYPE_LABELS} />
          ...
          <Field label="見積期限" name="bid_at" type="datetime-local" defaultValue={toDateTimeLocal(tender?.bid_at)} />
          <Field label="必要な参加資格" name="required_qualification" defaultValue={tender?.required_qualification ?? ""} />
          ...
          <Checkbox label="防衛省・自衛隊系案件" name="is_defense" defaultChecked={tender?.is_defense ?? false} />
        </div>
