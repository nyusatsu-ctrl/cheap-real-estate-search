...
export async function getDiagnosisV22FunnelSummary(): Promise<DiagnosisV22FunnelSummary | null> {
  ...
  const { data, error } = await supabase
    .from("construction_diagnosis_sessions")
    .select("primary_trade, device_type, browser_family, short_completed_at, detailed_started_at, detailed_completed_at, abandoned_stage, abandoned_question_id, diagnosis_id");
  if (error || !data) return null;
