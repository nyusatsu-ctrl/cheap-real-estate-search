...
const diagnosisFooterLinks = [
  { href: "/diagnosis/terms", label: "利用規約" },
  { href: "/diagnosis/privacy", label: "プライバシーポリシー" },
  { href: "/diagnosis/disclaimer", label: "免責事項" },
  { href: "/contact", label: "お問い合わせ" }
];
...
export function AppFooter() {
  ...
  const isDiagnosis = pathname.startsWith("/diagnosis") || pathname.startsWith("/construction-sales-diagnosis") || pathname.startsWith("/admin/diagnoses");
  const links = isDiagnosis
    ? diagnosisFooterLinks
    : pathname.startsWith("/tenders") || pathname.startsWith("/favorites") || pathname.startsWith("/notifications")
      ? tenderFooterLinks
      : footerLinks;
  ...
  return (
    <footer className="border-t border-slate-200 bg-white">
      <div className="mx-auto flex max-w-6xl flex-col gap-3 px-4 py-6 text-sm text-slate-600 md:flex-row md:items-center md:justify-between">
        <p className="font-semibold text-slate-700">© {BUSINESS_INFO.companyName}{isDiagnosis ? "｜エコループ 建設会社向け経営診断" : ""}</p>
