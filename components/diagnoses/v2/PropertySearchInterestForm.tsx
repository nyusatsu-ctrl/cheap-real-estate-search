...
export function PropertySearchInterestForm({ sessionId, companyName = "", email = "" }: { sessionId: string; companyName?: string; email?: string }) {
  ...
  return (
    <section className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm print:hidden">
      <h2 className="text-xl font-black text-slate-950">建設会社にも活用できる格安不動産情報サービスを開発中です</h2>
      <p className="mt-3 text-sm leading-7 text-slate-700">全国の格安・低価格不動産をまとめて探せるサービスを開発しています。事務所、資材置場、購入後の解体・改修、投資物件等の情報収集に利用できる予定です。</p>
      <form action={action} className="mt-5">
        <input type="hidden" name="session_id" value={sessionId} />
        <fieldset><legend className="text-sm font-black text-slate-950">完成時に案内を希望しますか。</legend><div className="mt-3 grid gap-2">
