...
const SCAFFOLD_QUESTIONS: DiagnosisV2Question[] = [
  ...
  question("SC05", "sales", "売上が1社に偏りすぎていないか、支払い条件や急な変更も確認していますか。", 2, [option(0, "偏りも条件も分からない"), option(1, "1社に大きく偏っている"), option(2, "分かっているが対策していない"), option(3, "取引先を増やし、条件も確認"), option(4, "利益、入金、職人の予定で仕事を受けるか決める")])
];
...
const INTERIOR_QUESTIONS: DiagnosisV2Question[] = [
  ...
  question("IN03", "sales", "売上が特定の工務店や管理会社に偏りすぎていないか分かっていますか。", 2, [option(0, "どれだけ偏っているか分からない"), option(1, "1社に大きく偏っている"), option(2, "分かっているが対策していない"), option(3, "取引先を増やしている"), option(4, "金額、支払い条件、利益を見て取引先を選ぶ")]),
...
const COMMON_SPECIALTY_QUESTIONS: DiagnosisV2Question[] = [
  question("SP01", "profit", "材料代、職人代、外注費、機械や車の費用を、工事ごとに計算していますか。", 3, [
    option(0, "費用を計算していない"),
    option(1, "大きな費用だけ計算している"),
    option(2, "工事によって分けて計算している"),
    option(3, "ほとんどの工事で費用ごとに計算している"),
    option(4, "実際にかかった費用を次の見積もりに使っている")
  ]),
  question("SP02", "technical", "現場や職人ごとの作業量、仕上がり、手直しの多さを分かっていますか。", 2, [
    option(0, "全く分かっていない"),
    option(1, "社長や担当者の感覚だけで判断している"),
    option(2, "問題が起きた時だけ確認している"),
    option(3, "作業量、仕上がり、手直しを記録している"),
    option(4, "記録を職人の配置、見積もり、教育に使っている")
  ]),
  question("SP03", "sales", "売上が特定の取引先に偏りすぎていないか、支払い条件も含めて確認していますか。", 2, [
    option(0, "売上の偏りも支払い条件も分からない"),
    option(1, "1社に大きく偏っている"),
    option(2, "偏りは分かるが対策していない"),
    option(3, "取引先を増やし、支払い条件も確認している"),
    option(4, "売上、利益、入金時期を見て仕事を受けるか決めている")
  ]),
  question("SP04", "organization", "仕事量に対して、職人、資格を持つ人、協力会社が足りているか分かっていますか。", 2, [
    option(0, "全く分かっていない"),
    option(1, "いつも人が足りない"),
    option(2, "今ある仕事に必要な人数だけは分かる"),
    option(3, "今後の仕事にも対応できるか把握している"),
    option(4, "仕事量に合わせて職人や協力会社の予定を組んでいる")
  ]),
  question("SP05", "control", "その工事に必要な法律、安全、道具、記録を、工事ごとに管理していますか。", 2, [
    option(0, "必要な法律、安全、道具、記録を確認していない"),
    option(1, "問題が起きた時だけ確認している"),
    option(2, "主な項目だけ確認している"),
    option(3, "ほとんどの工事で確認し、記録している"),
    option(4, "記録を確認し、事故や漏れを防ぐ対策に使っている")
  ])
];
