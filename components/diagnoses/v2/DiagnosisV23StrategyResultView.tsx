...
export function DiagnosisV23StrategyResultView({
  ...
  return (
    <div className="diagnosis-print-page bg-slate-50">
      <section className="border-b border-slate-200 bg-white"><div className="mx-auto max-w-5xl px-4 py-8"><p className="text-sm font-black text-brand-700">御社の再成長戦略</p><h1 className="mt-2 text-3xl font-black text-slate-950">{companyName ? `${companyName}様` : "診断結果に基づく優先戦略"}</h1><p className="mt-3 max-w-3xl text-sm leading-7 text-slate-700">回答内容を根拠に、最初に行うことと今後90日間の進め方を整理しました。売上や受注を保証するものではありません。</p>{saved && !printMode ? <div className="mt-5"><DiagnosisPrintLauncher diagnosisId={id} /></div> : null}</div></section>
      <div className="mx-auto max-w-5xl space-y-6 px-4 py-8">
        ...
        <Section title="エコループの支援判定" icon={<CheckCircle2 className="h-5 w-5 text-brand-700" />}><p className="text-xl font-black text-slate-950">{result.supportJudgment}</p><p className="mt-3 text-sm leading-7 text-slate-700">すべての会社に支援が必要とは判定していません。自社で進められる項目と、個別確認が必要な項目を分けて判断してください。</p>{saved && !printMode ? <Link href={`/diagnosis/consultation/${id}`} className="mt-4 inline-flex rounded bg-brand-700 px-5 py-3 text-sm font-black text-white">30分の個別相談を申し込む</Link> : null}</Section>
        {!printMode && !saved ? <DiagnosisV23ResultActions sessionId={id} /> : null}
        {!printMode ? <PropertySearchInterestForm sessionId={id} companyName={companyName} email={email} /> : null}
